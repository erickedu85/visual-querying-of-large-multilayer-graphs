...
public class Dijkstra {
	...
	/**
	 * @param graph
	 *            A graph with Vertex and Edges
	 */
	public Dijkstra(Graph graph) {
		// create a copy of the array so that we can operate on this array
		this.nodes = new ArrayList<Vertex>(graph.getListNode());
		this.edges = new ArrayList<Edge>(graph.getListEdge());
	}
	...
	private List<Vertex> getNeighbors(Vertex node) {
		...
		for (Edge edge : edges) {
			// new
//			int indexDestination = GraphUtil.getIndexNode(nodes, edge.getIdTarget());
			Vertex nodeDestination = GraphUtil.getNodeById(nodes, edge.getIdTarget()); // nodes.get(indexDestination);
			// end-new
	...
	private List<Vertex> getNeighborsAll(Vertex node) {
		List<Vertex> neighbors = new ArrayList<Vertex>();
		for (Edge edge : edges) {
//			int indexDestination = GraphUtil.getIndexNode(nodes, edge.getIdSource());
			Vertex nodeDestination = GraphUtil.getNodeById(nodes, edge.getIdSource()); // nodes.get(indexDestination);
