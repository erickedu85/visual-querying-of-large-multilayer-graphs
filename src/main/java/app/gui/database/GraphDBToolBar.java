...
public class GraphDBToolBar {
	...
	public static ComboBox<String> cmbLabelNodesPatterns;
	private Scene scene;
	...
	private ToolBar addToolBar() {
		...
		cmbLabelNodesPatterns = new ComboBox<String>();
		...
		cmbLabelNodesPatterns.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				try {
					...
					processingApp.searchNodePatternByLabel(value);
		...
		double space = Application.SCREEN_WIDTH - Constants.LAYOUT_QUERY_VIEW_WIDTH - Constants.LAYOUT_EMBEDDINGS_VIEW_WIDTH
				- Constants.VISUAL_GRAPH_DB_CMBLABELNODES_WIDTH - (2*Constants.VISUAL_GRAPH_DB_BTN_WIDTH);

		int spacerWidth = (int) (space - 80); // vast: 120 //SSDBM: 140
