...
//		 layoutGrid.setPrefHeight(Application.SCREEN_HEIGHT);
	...
	private Canvas createLeyendaListEmbedding() {
		...
		int lenghtR = Constants.LAYOUT_EMBEDDINGS_VIEW_WIDTH - 30;
		int heightR = 40;
		int padding = 50;
		int diametreOval = 25;
		...
		for (int i = 0; i < processingApp.graph.getListEmbeddingsNodeNumAppears().size(); i++) {
			int numeroValue = (int) processingApp.graph.getListEmbeddingsNodeNumAppears().get(i);
			if (numeroValue > max)
				max = numeroValue;
			if (numeroValue < min)
				min = numeroValue;
	...
	private HBox addBottomBox() {
		...
		// Btn clear kelps
		Button btnClearKelps = ComponentCreator.makeButton("Clear", "", ContentDisplay.TEXT_ONLY,
		...
		btnClearKelps.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				...
				processingApp.graph.setShowEmbeddingLabels(true);
			}
		});
