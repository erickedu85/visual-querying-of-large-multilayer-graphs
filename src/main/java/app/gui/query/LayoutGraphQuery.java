...
public class LayoutGraphQuery {
	...
	public static GraphQuerying designQuery;// handle action
	private Scene scene;
	...
	public LayoutGraphQuery() {
		...
		GraphQuerying.parent = this;
		layoutGraphQueryView = new BorderPane();
		layoutGraphQueryView.setMinHeight(Constants.VISUAL_GRAPH_QUERY_CANVAS_HEIGHT
				+ Constants.VISUAL_GRAPH_QUERY_BTN_HEIGHT + Constants.VISUAL_GRAPH_QUERY_STATUS_HEIGHT);
		layoutGraphQueryView.setMaxHeight(Constants.VISUAL_GRAPH_QUERY_CANVAS_HEIGHT
				+ Constants.VISUAL_GRAPH_QUERY_BTN_HEIGHT + Constants.VISUAL_GRAPH_QUERY_STATUS_HEIGHT);
		layoutGraphQueryView.setPrefHeight(Constants.VISUAL_GRAPH_QUERY_CANVAS_HEIGHT
				+ Constants.VISUAL_GRAPH_QUERY_BTN_HEIGHT + Constants.VISUAL_GRAPH_QUERY_STATUS_HEIGHT);
		...
		double heightLayoutHistogram = Application.SCREEN_HEIGHT - (Constants.VISUAL_GRAPH_QUERY_CANVAS_HEIGHT
				+ Constants.VISUAL_GRAPH_QUERY_BTN_HEIGHT + Constants.VISUAL_GRAPH_QUERY_STATUS_HEIGHT) - 100; // -60 // 110 normal
	...
	public void updateGhostButton() {
		Platform.runLater(new Runnable() {
			public void run() {
				GraphQuerying.tbGhost.setDisable(false);
				GraphQuerying.tbGhost.setSelected(false);
			}
		});
	}
	...
	public void updateSearchBtn() {
		Platform.runLater(new Runnable() {
			public void run() {
				GraphQuerying.tbSearch.setGraphic(new ImageView(new Image(this.getClass().getClassLoader()
						.getResourceAsStream(Constants.RESOURCES_IMAGE_PATH.concat("search.png")))));
				GraphQuerying.tbSearch.setText("Search");
				GraphQuerying.tbSearch.setSelected(false);
			}
		});
	}
