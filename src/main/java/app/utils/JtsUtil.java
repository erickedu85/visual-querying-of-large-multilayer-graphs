...
public class JtsUtil {
	 ...
	 */
	public static Vertex getPointIntersectionNodeToEdge(Vertex node, Edge edge, Vertex nodeReference) {
		...
		Geometry geometryNode = new GeometryFactory()
				...
				.buffer(node.getDiameter() / 2 - 1.5); //1.5
		...
		if (geometryEdge.contains(geometryNode)) {
			...
			int type = Constants.GRAPH_DB_NODE_PATTERN_FIXED_TYPE;
			...
			Vertex vertexIntersection = new Vertex(id, label, type, nodePosition, stroke, fill, isLabelled, isVisible,
					diameter);
			...
			Geometry geometryNodeIntersection = new GeometryFactory()
					.createPoint(new Coordinate(vertexIntersection.getPosition().getX1(),
							vertexIntersection.getPosition().getY1()))
					.buffer(vertexIntersection.getDiameter() / 2 - 1.5);

//			logger.info("Node:" + node + " Intersect: " + edge);
//			logger.info(geometryNode);
//			logger.info(geometryEdge);
//			logger.info("Polygon Intersection: " + pointsIntersection.getNumGeometries());
